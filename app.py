...
def all_leads():
    ...
    return render_template('all_leads.html', leads=leads, now=now, users=users, search=search)@app.route('/leads/export')
@login_required
@admin_required
def export_leads():
...
def get_next_sales_assignee():
    """Round-robin assignment among active sales staff: Aslam, Saada, Anfal"""
    sales_staff = User.query.filter(
        User.active == True,
        User.name.in_(['Aslam', 'Saada', 'Anfal'])
    ).all()
    
    if not sales_staff:
        return None
    
    last_lead = Lead.query.filter(
        Lead.source == 'Social Media',
        Lead.assigned_to.in_([u.name for u in sales_staff])
    ).order_by(Lead.created_at.desc()).first()
    
    names = [u.name for u in sales_staff]
    
    if not last_lead or last_lead.assigned_to not in names:
        return names[0]
    
    current_index = names.index(last_lead.assigned_to)
    ...
    try:
        for entry in data.get('entry', []):
            for change in entry.get('changes', []):
                ...
                lead_id_meta = change['value'].get('leadgen_id')
                ...
                service = field_data.get('type_of_service', field_data.get('service', ''))
                
                if phone and not phone.startswith('+'):
                    phone = '+971' + phone.lstrip('0')
                
                assigned = get_next_sales_assignee()
                ...
                new_lead = Lead(
                    ...
                    followup_date=datetime.utcnow().date() + timedelta(days=1)
                )
                db.session.add(new_lead)
                db.session.flush()
                
                remark_text = f'Meta Lead Form. Email: {email}' if email else 'Meta Lead Form'
                update = LeadUpdate(
                    lead_id=new_lead.id,
                    stage='New',
                    remark=remark_text,
        ...
        db.session.rollback()
        app.logger.error(f'Meta webhook error: {e}')
    
    return 'OK', 200
# ─── END META LEAD WEBHOOK ───────────────────────────────────────────
